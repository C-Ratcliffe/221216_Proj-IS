...
meas_thick <- c(meas[1], meas[4], meas[7], meas[10], meas[13])
meas_cort <- c(meas[2], meas[5], meas[8], meas[11], meas[14])
meas_sub <- c(meas[3], meas[6], meas[9], meas[12], meas[15])
...
for (i in seq(2, 5, 1)){
	for (j in seq(1, 148, 1)){
		data <- cbind(meas_thick[[1]][,j], meas_thick[[i]][,j])
...
for (i in seq(2, 5, 1)){
	for (j in seq(1, 148, 1)){
		data <- cbind(meas_cort[[1]][,j], meas_cort[[i]][,j])
...
for (i in seq(2, 5, 1)){
	for (j in seq(1, 30, 1)){
		data <- cbind(meas_sub[[1]][,j], meas_sub[[i]][,j])
...
cortvol_raw <- gather(data = meas_cort[[1]])
cortvol_an <- gather(data = meas_cort[[2]])
cortvol_dl <- gather(data = meas_cort[[3]])
cortvol_res <- gather(data = meas_cort[[4]])
cortvol_ssr <- gather(data = meas_cort[[5]])

cortvol_all <- cbind(cortvol_raw, cortvol_an[,2], cortvol_dl[,2], cortvol_res[,2], cortvol_ssr[,2])
...
filename <- "F:/Working_Directory_Projects/Work/221216_Proj-IS/materials/figure-6/vol_nobg"
...
filename <- "F:/Working_Directory_Projects/Work/221216_Proj-IS/materials/figure-6/vol_blackbg"
#ggsave(plot = plot.cort, device = 'svg', scale = 1, units = 'px', filename = paste(filename, 'svg', sep = '.'), dpi=300)
ggsave(plot = plot.cort, device = 'png', height = 1200, width = 4800, units = 'px', filename = paste(filename, 'png', sep = '.'))

#Subcortical Volume Correlations####

subvol_raw <- gather(data = meas_sub[[1]])
subvol_an <- gather(data = meas_sub[[2]])
subvol_dl <- gather(data = meas_sub[[3]])
subvol_res <- gather(data = meas_sub[[4]])
subvol_ssr <- gather(data = meas_sub[[5]])

subvol_all <- cbind(subvol_raw, subvol_an[,2], subvol_dl[,2], subvol_res[,2], subvol_ssr[,2])
...
rm(subvol_raw, subvol_an, subvol_dl, subvol_res, subvol_ssr, subvol_all)
...
filename <- "F:/Working_Directory_Projects/Work/221216_Proj-IS/materials/figure-6/sub_nobg"
...
filename <- "F:/Working_Directory_Projects/Work/221216_Proj-IS/materials/figure-6/sub_blackbg"
#ggsave(plot = plot.sub, device = 'svg', scale = 1, units = 'px', filename = paste(filename, 'svg', sep = '.'), dpi=300)
ggsave(plot = plot.sub, device = 'png', height = 1200, width = 4800, units = 'px', filename = paste(filename, 'png', sep = '.'))

## Cortical Thickness Correlations####

cortthick_raw <- gather(data = meas_thick[[1]])
cortthick_an <- gather(data = meas_thick[[2]])
cortthick_dl <- gather(data = meas_thick[[3]])
cortthick_res <- gather(data = meas_thick[[4]])
cortthick_ssr <- gather(data = meas_thick[[5]])

cortthick_all <- cbind(cortthick_raw, cortthick_an[,2], cortthick_dl[,2], cortthick_res[,2], cortthick_ssr[,2])
...
rm(cortthick_raw, cortthick_an, cortthick_dl, cortthick_res, cortthick_ssr, cortthick_all)
...
filename <- "F:/Working_Directory_Projects/Work/221216_Proj-IS/materials/figure-6/thick_nobg"
...
filename <- "F:/Working_Directory_Projects/Work/221216_Proj-IS/materials/figure-6/thick_blackbg"
#ggsave(plot = plot.thick, device = 'svg', scale = 1, units = 'px', filename = paste(filename, 'svg', sep = '.'), dpi=300)
ggsave(plot = plot.thick, device = 'png', height = 1200, width = 4800, units = 'px', filename = paste(filename, 'png', sep = '.'))
...
filename <- "F:/Working_Directory_Projects/Work/221216_Proj-IS/materials/figure-6/archive/example_scatter_nobg"
...
filename <- "F:/Working_Directory_Projects/Work/221216_Proj-IS/materials/figure-6/archive/example_scatter_blackbg"
ggsave(plot = plot.egscatter, device = 'svg', scale = 1, units = 'px', filename = paste(filename, 'svg', sep = '.'), dpi=300)
#ggsave(plot = plot.egscatter, device = 'png', scale = 1, units = 'px', filename = paste(filename, 'png', sep = '.'), dpi=300)
